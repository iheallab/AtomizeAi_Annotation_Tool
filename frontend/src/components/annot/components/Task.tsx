      ...
      setVariableValidity((prev) => {
        return prev.map((q, qIndex) =>
          qIndex === questionIndex
            ? q.map((t, tIndex) =>
                tIndex === taskIndex
                  ? t.map(() => questionValid) // ✅ If question is valid, all are valid
                  : t
              )
            : q
        );
      ...
      children: variables.map((variable, variableIndex) => ({
        title: (
          <span
            ...
            style={{
              color: variableValidity[questionIndex]?.[taskIndex]?.[
                variableIndex
              ]
                ? "#263238"
                : "#b0bec5",
          ...
          >
            <Switch
              ...
              onChange={() => handleVariableToggle(variableIndex)}
              style={{
                backgroundColor: variableValidity[questionIndex]?.[taskIndex]?.[
                  variableIndex
                ]
                  ? "#607d8b"
                  : "#b0bec5",
